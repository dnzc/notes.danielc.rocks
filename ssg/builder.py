...
def add_link_anchors(page, cur_target_dir, h1=False): 
    ...
    """
    page = re.sub(r'<h2 id="(.*?)">(.*?)</h2>', r'<h2 className="group flex space-x-1 items-baseline"><span id="\1">\2</span><Pilcrow href="#\1" text="https://notes.danielc.rocks'+cur_target_dir+r'#\1"/></h2>', page, flags=re.DOTALL)
    if h1: page = re.sub(r'<h1 id="(.*?)">(.*?)</h1>', r'<h1 className="group flex space-x-1 items-baseline"><span id="\1">\2</span><Pilcrow href="#\1" text="https://notes.danielc.rocks'+cur_target_dir+r'#\1"/></h1>', page, flags=re.DOTALL)
    return page
